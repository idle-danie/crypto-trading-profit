...
def process_kafka_stream(df: DataFrame) -> DataFrame:
    # Kafka에서 들어온 데이터를 'crypto_prices' 테이블 형식으로 변환
    df = df.withColumnRenamed("last_price", "price").withColumn("timestamp", F.current_timestamp())
    return df
...
def calculate_arbitrage_average(df: DataFrame) -> DataFrame:
    ...
    # 워터마크를 설정하여 30초 동안의 지연 데이터 허용 (timestamp 기준)
...
def calculate_arbitrage_values(df: DataFrame) -> DataFrame:
    ...
    # 각 거래소 쌍별로 symbol, timestamp, 그리고 위에서 계산된 profit_value를 포함한 DataFrame 생성
    arbitrage_values_df = profit_df.select(
        ...
        F.explode(
            F.array(
                F.struct(
                    F.lit("Binance-Upbit").alias("exchange_pair"),
                ...
                F.struct(
                    F.lit("Binance-Bithumb").alias("exchange_pair"),
                ...
                F.struct(
                    F.lit("Kucoin-Upbit").alias("exchange_pair"), F.col("profit_kucoin_upbit_btc").alias("profit_value")
                ),
                F.struct(
                    F.lit("Kucoin-Upbit").alias("exchange_pair"), F.col("profit_kucoin_upbit_eth").alias("profit_value")
                ),
                F.struct(
                    F.lit("Kucoin-Bithumb").alias("exchange_pair"),
